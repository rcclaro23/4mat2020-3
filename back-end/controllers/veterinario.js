...
controller.novo = async (req, res) => {
    try{
        ...
        await Veterinario.create(req.body)
        // HTTP 201: Created
        res.status(201).end()
    }
    catch(erro){
...
controller.listar = async (req, res) => {
    ...
    let dados = await Veterinario.find().populate('secretaria') 
...
controller.obterUm = async(req, res) => {
    ...
    let obj = await Veterinario.findById(id)
...
controller.atualizar = async (req, res) => {
    ...
    let obj = await Veterinario.findByIdAndUpdate(id, req.body)
...
controller.excluir = async (req, res) => {
    try{
        ...
        const id = req.body._id
        let obj = await Veterinario.findByIdAndDelete(id)
