...
export class ClienteService {
    ...
    private apiUri = this.server + 'cliente'
    
    constructor(private http : HttpClient) { }

    listar(){
        return this.http.get(this.server + 'cliente').toPromise()
    }

    excluir(id : string) {
        return this.http.request('DELETE', this.server + 'cliente', {body: {_id: id}}).toPromise()
    }
