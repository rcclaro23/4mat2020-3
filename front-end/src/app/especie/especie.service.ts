...
export class EspecieService {
    ...
    private apiUri = this.server + 'especie'
    
    constructor(private http : HttpClient) { }

    listar(){
        return this.http.get(this.server + 'especie').toPromise()
    }

    excluir(id : string) {
        return this.http.request('DELETE', this.server + 'especie', {body: {_id: id}}).toPromise()
    }
