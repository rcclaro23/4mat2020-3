...
export class ConsultaService {
    ...
    private apiUri = this.server + 'consulta'
    
    constructor(private http : HttpClient) { }

    listar(){
        return this.http.get(this.server + 'consulta').toPromise()
    }

    excluir(id : string) {
        return this.http.request('DELETE', this.server + 'consulta', {body: {_id: id}}).toPromise()
    }
